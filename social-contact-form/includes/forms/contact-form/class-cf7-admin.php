...
if ( ! class_exists(__NAMESPACE__ . '\Admin') ) {
	...
	class Admin extends \FormyChat\Base {
		...
		public function add_actions() {
			...
			add_action('wpcf7_admin_footer', [ $this, 'formychat_destination_toggle_script' ]);
		}
		...
		public function formychat_cf7_tab_callback( $form ) {
			...
			$form_options = [
				...
				'destination_type' => [ 'formy_chat_destination_type', 'phone' ],
				'group_invite_code' => [ 'formy_chat_group_invite_code', '' ],
			...
			<fieldset>
				...
				<table class="form-table formychat-settings-cf7">
					<tbody>
						...
						<!-- Destination Type (Phone / Group) -->
						<tr>
							<th scope="row">
								<div>
									<label><?php esc_html_e('Destination', 'social-contact-form'); ?></label>
									<?php $this->print_tooltip('Send to a phone number or a WhatsApp group.'); ?>
								</div>
							</th>
							<td>
								<label class="formychat-inline">
									<input type="radio" name="formychat_destination_type" value="phone" <?php checked( 'phone', $formychat['destination_type'] ); ?> />
									<?php esc_html_e('Phone', 'social-contact-form'); ?>
								</label>
								<label class="formychat-inline">
									<input type="radio" name="formychat_destination_type" value="group" <?php checked( 'group', $formychat['destination_type'] ); ?> <?php echo ! $is_ultimate ? 'disabled' : ''; ?> />
									<?php esc_html_e('Group', 'social-contact-form'); ?>
									<?php if ( ! $is_ultimate ) : ?>
										<span class="formychat-pro-badge">Pro</span>
									<?php endif; ?>
								</label>
							</td>
						</tr>

						<!-- WhatsApp Number (when Phone) / Group Invite (when Group) -->
						<tr class="formychat-dest-phone" style="<?php echo 'group' === $formychat['destination_type'] ? 'display:none' : 'display:table-row'; ?>">
							<th scope="row">
								<div>
						...
						<tr class="formychat-dest-group" style="<?php echo 'phone' === $formychat['destination_type'] ? 'display:none' : 'display:table-row'; ?>">
							<th scope="row">
								<div>
									<label for="formychat_group_invite_code"><?php esc_html_e('Group Invite Link', 'social-contact-form'); ?></label>
									<?php $this->print_tooltip('Paste your WhatsApp group invite link.'); ?>
								</div>
							</th>
							<td>
								<?php
                                formychat_group_invite_field( [
									'group_invite_code' => $formychat['group_invite_code'],
		...
		/**
		 * Output script to toggle phone/group field visibility based on destination.
		 *
		 * @return void
		 */
		public function formychat_destination_toggle_script() {
			...
			<script>
			(function(){
				function formychatCf7Toggle() {
					var container = document.querySelector('.formychat-settings-cf7');
					if (!container) return;
					var radios = container.querySelectorAll('input[name="formychat_destination_type"]');
					if (!radios.length) return;
					function toggle() {
						var checked = container.querySelector('input[name="formychat_destination_type"]:checked');
						var v = checked ? checked.value : 'phone';
						container.querySelectorAll('.formychat-dest-phone').forEach(function(el){
							el.style.display = (v === 'phone') ? 'table-row' : 'none';
						});
						container.querySelectorAll('.formychat-dest-group').forEach(function(el){
							el.style.display = (v === 'group') ? 'table-row' : 'none';
						});
					}
					radios.forEach(function(el){ el.addEventListener('change', toggle); });
					toggle();
				}
				if (document.readyState === 'loading') {
					document.addEventListener('DOMContentLoaded', formychatCf7Toggle);
				} else {
					formychatCf7Toggle();
				}

				/**
				 * Patch: exclude formychat fields from third-party jQuery validate instances
				 * (e.g. wpcf7-redirect plugin) so they don't trigger spurious validation errors.
				 */
				function formychatPatchJQueryValidate() {
					if ( typeof jQuery === 'undefined' || typeof jQuery.validator === 'undefined' ) {
						return;
					}
					var $form = jQuery('#wpcf7-contact-form-editor form');
					if ( ! $form.length ) return;
					var validator = $form.data('validator');
					if ( ! validator ) return;
					var currentIgnore = validator.settings.ignore || ':hidden';
					if ( currentIgnore.indexOf('.formychat-no-validate') === -1 ) {
						validator.settings.ignore = currentIgnore + ', .formychat-no-validate';
					}
				}

				// Run after a short delay to allow wpcf7-redirect to initialise its validator first.
				if ( document.readyState === 'loading' ) {
					document.addEventListener('DOMContentLoaded', function(){ setTimeout(formychatPatchJQueryValidate, 200); });
				} else {
					setTimeout(formychatPatchJQueryValidate, 200);
				}
			})();
			</script>
			<?php
		...
		public function save_contact_form( $cf7 ) {
			...
			$formychat = array(
				...
				'destination_type' => isset($_POST['formychat_destination_type']) ? sanitize_text_field( wp_unslash($_POST['formychat_destination_type'])) : 'phone',
				'group_invite_code' => isset($_POST['formychat_group_invite_code']) ? sanitize_text_field( wp_unslash($_POST['formychat_group_invite_code'])) : '',
