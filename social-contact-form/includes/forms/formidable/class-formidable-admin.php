...
if ( ! class_exists(__NAMESPACE__ . '\Admin') ) {
    ...
    class Admin extends \FormyChat\Base {
        ...
        public function render_formychat_tab( $values ) {
            ...
            $destination_type = isset($form_options['formychat_destination_type']) ? $form_options['formychat_destination_type'] : 'phone';
            $group_invite_code = isset($form_options['formychat_group_invite_code']) ? $form_options['formychat_group_invite_code'] : '';
...
<div class="frm-form-settings-section formychat-formidable-settings" id="html_settings">
    ...
    <fieldset>
        ...
        <label><input type="radio" name="formychat_destination_type" value="phone" <?php checked($destination_type, 'phone'); ?> /> <?php esc_html_e('Phone', 'social-contact-form'); ?></label>
        <label style="margin-left:15px"><input type="radio" name="formychat_destination_type" value="group" <?php checked($destination_type, 'group'); ?> <?php echo $is_ultimate ? '' : 'disabled'; ?> /> <?php esc_html_e('Group', 'social-contact-form'); ?><?php echo $is_ultimate ? '' : ' <span class="formychat-pro-badge">Pro</span>'; ?></label>
    </fieldset>

    <fieldset class="formychat-dest-phone" style="<?php echo 'group' === $destination_type ? 'display:none' : ''; ?>">
    ...
    <fieldset class="formychat-dest-group" style="<?php echo 'phone' === $destination_type ? 'display:none' : ''; ?>">
        <label><?php esc_html_e('Group Invite Link', 'social-contact-form'); ?></label>
            <?php
    ...
    <fieldset>
        ...
        public function add_formychat_field_options( $options, $values ) {
            ...
            $options['formychat_destination_type'] = isset($values['formychat_destination_type']) ? $values['formychat_destination_type'] : 'phone';
            $options['formychat_group_invite_code'] = isset($values['formychat_group_invite_code']) ? $values['formychat_group_invite_code'] : '';
         ...
         */
        /**
         * Add destination type toggle script.
         *
         * @since 1.0.0
         */
        public function add_destination_toggle_script() {
            ?>
            <script>
            (function(){
                var r = document.querySelectorAll('.formychat-formidable-settings input[name="formychat_destination_type"]');
                if (!r.length) return;
                function toggle() {
                    var checked = document.querySelector('.formychat-formidable-settings input[name="formychat_destination_type"]:checked');
                    var v = checked ? checked.value : 'phone';
                    document.querySelectorAll('.formychat-formidable-settings .formychat-dest-phone').forEach(function(el){ el.style.display = v === 'phone' ? '' : 'none'; });
                    document.querySelectorAll('.formychat-formidable-settings .formychat-dest-group').forEach(function(el){ el.style.display = v === 'group' ? '' : 'none'; });
                }
                r.forEach(function(el){ el.addEventListener('change', toggle); });
                toggle();
            })();
            </script>
            <?php
        ...
        public function add_formychat_icon() {
            $this->add_destination_toggle_script();
            echo '<style>
